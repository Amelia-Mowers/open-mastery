# Cairn — Architecture v0.8

*OpenMastery project. Supersedes v0.5. Resolves the v0.5 review, replaces per-student key wrapping with conventional at-rest handling, and resolves the v0.7 review (policy versioning, clock as events, accounting rules, outbox semantics).*
...
- **Homework / at-home mode, on any deployment.** Guide-in-the-room is an invariant; hosted sites enforce it via device enrollment (§7).
...
6. Guide in the room: student sessions only from guide-enrolled devices.
...
// siteSeq is assigned by the site server on ingest (single writer) and is the total order.
// (deviceId, deviceSeq) makes client retries idempotent. state = fold(core, events by siteSeq).
...
type Event = Envelope & (
  ...
  | { kind: 'llm_help', itemId, turns }                 // marks the item assisted
...
```
`p` is a read model and may shift when BKT parameters are retuned; mastery is an event and does not. The fold tolerates `siteSeq` gaps (deletion). `assisted` is scoped per (student, itemId, paramHash). FSRS due dates are durations from the granting event, evaluated against **site time** (§7), not raw wall-clock. The fold does not read policy constants, so policy changes never alter replayed state.
...
**Mastery model.** BKT per skill. Assisted attempts (hint level *k* > 0 or `llm_help`) update with an inflated guess parameter `G_eff = 1 − (1−G)·0.5^k`, so correct-with-help carries reduced evidence; `S` is unchanged, so wrong-with-help is full negative evidence (intended: help should not make failure ambiguous). The mastery **check** is *offered* when `p ≥ 0.9` or after 3 consecutive unassisted correct; **passing 2 unassisted isomorphs grants mastery regardless of `p`** (emits `mastery_granted`, sets `p ≥ 0.95`). Assisted attempts never satisfy the check. This is a deliberately thin bar; FSRS review is the intended safety net for false positives.
...
**Item selector.** Eligible = unmastered skills with prereqs mastered ∪ FSRS-due. Interleave ~1 review per 3 new items; target 70–90% expected correctness in `practice` and `review` phases only (lesson/faded are exempt), where in v1 expected correctness is the per-skill BKT prediction (no per-item term; `difficulty` only orders items within a skill); avoid recent items. Runtime isomorphs come **only** from `generator` (total, grader-verified). LLM isomorph drafting is pipeline-only. Check items are generator-backed, non-choice widgets.

**Corrective policy (invariant).** After **3 misses** in a skill within a session: hint → alternative explanation with a `representation` not yet viewed → prerequisite probe → `guide_flag`. Hard cap of **6 attempts per skill per session**, after which the skill is parked and flagged.
...
- PWA, React + TypeScript, Tailwind; installable so the shell loads instantly; all state lives on the site server.
...
- **Device enrollment:** a guide enrolls each device once (QR from the guide dashboard → device token). Student class-code + PIN logins are accepted only from enrolled devices; an unenrolled device sees only the enrollment screen. This is how hosted sites stay "in the room." Rate-limited regardless.
...
- Parent report: monthly, generated from the log — skills mastered, sessions attended, guide interventions. No minutes on task.
...
- **Time:** ordering never depends on wall-clock (`siteSeq`). Wall-clock corrections (RTC at boot, NTP when reachable, guide-supplied hint at session start) are recorded as `clock_set` events; **site time** = last correction + monotonic elapsed, and everything time-based (FSRS due, session stamps) reads site time, so the fold is deterministic. Backward corrections are rejected by a monotonic guard; a forward jump cannot flood reviews because the interleave ratio bounds reviews per session. Reference BOM includes an RTC module.
...
- **LLM gateway (optional, online):** grounded explainer chat (source section as context), rubric grading with confidence, isomorph generation, explanation drafting. Provider abstraction: hosted API under no-training terms, or local model. No student PII sent.
...
- Student uses LLM help on an item → that item never counts toward mastery; a later unassisted isomorph does.
- Two misses on a skill → next presentation is an explanation with a `representation` not yet seen; the same item is never re-served.
...
- BKT/FSRS update invariants (monotonicity, bounds, idempotent replay by `siteSeq`).
...
- Site time: fold with `clock_set` events is deterministic; forward jump does not exceed per-session review bound; backward correction rejected.
...
1. Schema + validator; 3 widgets (numeric, expression, number line); 1 viz template; low-end device in CI.
...
5. Single-file Bun site server running core behind `SiteStore`; event ingest with `siteSeq`; client outbox; device enrollment + class-code/PIN auth; at-rest encryption; deletion with backup aging; WAL mirror and restore.
